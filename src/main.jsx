...
function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      <section className="satellite-section section" id="satellite"><div className="section-top"><div><SectionLabel number="02">EARTH OBSERVATION</SectionLabel><h2>THE SATELLITE<br /><em>VIEW</em></h2></div><p className="section-copy">Planet released imagery from before and after the event through its disaster response program. The scenes provide a partial, cloud-obstructed view of the corridor.</p></div><div className="metrics"><div className="metric"><strong>14</strong><span>TOTAL SCENES</span><small>PlanetScope collection</small></div><div className="metric"><strong>5</strong><span>PRE-EVENT SCENES</span><small>Acquired 27 May 2026</small></div><div className="metric"><strong>9</strong><span>POST-EVENT SCENES</span><small>Acquired 26 Aug 2026</small></div><div className="metric"><strong>~3.8 <sup>m</sup></strong><span>IMAGERY RESOLUTION</span><small>PlanetScope</small></div></div><Compare /><div className="caveat"><span className="caveat-icon">!</span><p><strong>READ WITH CARE</strong> Post-event imagery was acquired under substantial monsoon cloud cover. Satellite imagery should therefore be interpreted as <em>partial observation</em> rather than a complete flood map.</p></div></section>
      ...
      <section className="data-section section" id="data"><div className="data-copy"><SectionLabel number="06">OPEN METHODOLOGY</SectionLabel><h2>DATA &<br /><em>METHODS</em></h2><p>The imagery in this page comes from Planet Labs through its disaster response program and is distributed through Source Cooperative.</p><a className="dataset-link" href={datasetUrl} target="_blank" rel="noreferrer">OPEN DATASET <ExternalLink size={14} /></a></div><div className="method-list"><div><span>VISUAL IMAGERY</span><p>Rendered scenes for visual inspection of the landscape.</p></div><div><span>SURFACE REFLECTANCE</span><p>Pixels adjusted to represent surface conditions; used for the pre-event product.</p></div><div><span>TOA RADIANCE</span><p>Top-of-atmosphere sensor measurements; used for the post-event product. Not directly comparable numerically with reflectance.</p></div><div><span>UDM2 MASKS</span><p>Usable-data masks that flag clouds and other unusable pixels.</p></div><div><span>STAC METADATA</span><p>Structured records describing scene dates, footprints, assets, and quality metadata.</p></div></div></section>
    </main>
